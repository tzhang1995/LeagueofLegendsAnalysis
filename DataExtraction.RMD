...
---
# Introduction
Like most league of legends players, I have a love / hate relationship with this game. I love the game to death, but playing it is an absolute nightmare. Feeding into this dynamic is partially the champion balance, or lack thereof, in this game which leads to frustrations on my part. In an effort to better understand champion balance I wanted to do two things:

1. Establish a more robust champion win - rate statistic
2. Apply unsupervised statistical learning techniques to model and understand team compositions

The first thing I wanted to establish is a covariate champion win rate statistic. For example, truly degenerate combos like Lucian / Nami or Nidalee / Renekton make playing the game absolutely miserable, and I would like to show that such particular combinations have a greater effect on team win rate than either champion alone. 

For the second, I look back at clash games that my friends and I have played, and I want to understand (beyond mistakes or skill disparity) compositional weaknesses we may or may not have drafted.

To Do List:
1. Add elo system to professional games
...
matchData$summonerIds <- accounts.raw %>% 
  mutate(
    accounts = map(
      accounts,
      .f = function(accounts){
...
matchData$gameInfo.low <- matches %>% 
...
matchData$gameInfo.high <- matches %>% 
